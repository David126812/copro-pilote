...
import type { DossierStatus, Dossier } from "@/data/mockData";
...
const DossiersList = () => {
  ...
  const reminderCount = dossiersWithReminders.length;
  ...
  const filtered = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      if (sortBy === "urgency") return urgencyOrder[a.urgency as UrgencyLevel] - urgencyOrder[b.urgency as UrgencyLevel];
          ...
          <>
            ...
            {showAdvanced === "filter" && (
              <div className="mb-4 p-3 rounded-[12px] bg-secondary/50 border border-border space-y-2.5">
                    ...
                    {statusFilters.map((f) => (
                      <button
                        ...
                        className={`px-2.5 py-[5px] rounded-full text-[11px] font-semibold transition active:scale-[0.98] ${
                          filter === f.value ? "bg-foreground text-card" : "bg-card text-muted-foreground border border-border hover:bg-accent"
                        }`}
                      >
                        {f.label}
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {([
                      { v: "all" as UrgencyFilter, l: "Toutes" },
                      { v: "critique" as UrgencyFilter, l: "Critique", dot: "bg-destructive" },
                      { v: "urgent" as UrgencyFilter, l: "Urgent", dot: "bg-[hsl(28_87%_52%)]" },
                      { v: "normal" as UrgencyFilter, l: "Normal", dot: "bg-primary/40" },
                    ]).map((f) => (
                      <button
                        key={f.v}
                        ...
                        className={`px-2.5 py-[5px] rounded-full text-[11px] font-semibold transition active:scale-[0.98] flex items-center gap-1.5 ${
                          urgencyFilter === f.v ? "bg-foreground text-card" : "bg-card text-muted-foreground border border-border hover:bg-accent"
                        }`}
                      >
            ...
            {showAdvanced === "sort" && (
              <div className="mb-4 p-3 rounded-[12px] bg-secondary/50 border border-border">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {([
                    { v: "recent" as SortValue, l: "Plus récent" },
                    { v: "urgency" as SortValue, l: "Urgence" },
                    { v: "name" as SortValue, l: "Nom A-Z" },
                    { v: "oldest" as SortValue, l: "Plus ancien" },
                  ]).map((s) => (
                    <button
                      key={s.v}
                      onClick={() => setSortBy(s.v)}
                      className={`px-2.5 py-[5px] rounded-full text-[11px] font-semibold transition ${
                        sortBy === s.v ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground border border-border hover:bg-accent"
                      }`}
                    >
                      {s.l}
                    </button>
